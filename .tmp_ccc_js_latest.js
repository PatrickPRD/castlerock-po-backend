...
(() => {
  ...
  function renderTable() {
    ...
    if (!filteredItems.length) {
      tableBody.innerHTML = '<tr><td colspan="8" class="empty-state">No cost items found for the current filters.</td></tr>';
      return;
    }
    ...
    Object.keys(grouped).sort((a, b) => a.localeCompare(b)).forEach((type) => {
      ...
      groupItems.forEach((item) => {
        ...
        const detail = [
          ...
          `Latest: <strong>${comparison.latest_cost === null ? '-' : money(comparison.latest_cost)}</strong>`,
          `Last Updated: <strong>${formatDate(item.last_updated || item.updated_at)}</strong>`,
